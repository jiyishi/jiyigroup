...
@interface JYRegisterView () <UITextFieldDelegate>
{
    double _minHeight;
    UIButton *hqBtn;
    UIButton *zcBtn;
...
// 登录界面
@property (nonatomic,assign)JYRegisterViewType JYRegisterViewType;
...
// 忘记密码 获取验证码界面
@property (nonatomic,assign)JYRegisterViewTypeSMS JYRegisterViewTypeSms;
...
- (void)creatUI:(JYRegisterViewType ) JYRegisterViewType
{
    self.JYRegisterViewType = JYRegisterViewType;
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    if (size.width<321) {
        NSLog(@"4 4s 5 5s"); // 216
        if (_minHeight>WIN_HEIGHT-216-30 && self.JYRegisterViewType==0) {
            [self setViewY:WIN_HEIGHT-216-30 - _minHeight animation:YES];
        }
        else if (_minHeight>WIN_HEIGHT-64-216-30 && self.JYRegisterViewType==1) {
            [self setViewY:WIN_HEIGHT-64-216-30 - _minHeight animation:YES];
        }
    }
    else if (size.width<377){
        NSLog(@"6");  // 258
        if (_minHeight>WIN_HEIGHT-64-258 && self.JYRegisterViewType==0) {
            [self setViewY:WIN_HEIGHT-64-258 - _minHeight animation:YES];
        }
        else if (_minHeight>WIN_HEIGHT-258 && self.JYRegisterViewType==1){
            [self setViewY:WIN_HEIGHT-258 - _minHeight animation:YES];
        }
    }
    else if (size.width>410){
        NSLog(@"6p"); // 271
        if (_minHeight>WIN_HEIGHT-64-271 && self.JYRegisterViewType==0) {
            [self setViewY:WIN_HEIGHT-64-271 - _minHeight animation:YES];
        }
        else if (_minHeight>WIN_HEIGHT-271 && self.JYRegisterViewType==1){
...
- (instancetype )initwithFrame:(CGRect)frame
         JYRegisterViewTypeSMS:(JYRegisterViewTypeSMS)JYRegisterViewTypeSMS
...
{
    if ([self  initWithFrame:frame]) {
        [self creatZhaoHePassWithTitle:title plTitle:plTitle JYRegisterViewTypeSMS:JYRegisterViewTypeSMS];
        self.JYRegisterViewTypeSms = JYRegisterViewTypeSMS;
...
{
     // 找回密码 (界面)
    if (JYRegisterViewTypeSMS == JYRegisterViewTypeNoScanfSMS) {
        // 验证码
     ...
     // 输入手机号获取验证码界面
    else if (JYRegisterViewTypeSMS == JYRegisterViewTypeScanfPhoneSMS ){
...
- (void)hqBtnClick
{
    if (self.JYRegisterViewTypeSms == JYRegisterViewTypeScanfPhoneSMS && self.hqAction) {
    ...
    else if (self.JYRegisterViewTypeSms == JYRegisterViewTypeNoScanfSMS && self.hqAction)
    {
